...
int main(int argc, char** argv) {
	...
	GLuint sphereTextureUniform = glGetUniformLocation(program.getGLId(), "uSphereTexture");
	GLuint cloudTextureUniform = glGetUniformLocation(program.getGLId(), "uCloudTexture");
	...
	while(!done) {
		...
		glUniform1i(sphereTextureUniform, TEXTURE_SLOT_EARTH);
		glUniform1i(cloudTextureUniform, TEXTURE_SLOT_CLOUD);
		...
		for(auto randomDirection : moonParameters)
		{
			...
			glUniform1i(sphereTextureUniform, TEXTURE_SLOT_MOON);
			glUniform1i(cloudTextureUniform, TEXTURE_SLOT_CLOUD);
